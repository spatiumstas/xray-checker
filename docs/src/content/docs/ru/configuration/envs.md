...
При настройке Telegram отправляет уведомление при первом неуспешном подключении, а затем при каждом восстановлении или повторной потере соединения. Успешная первая проверка не создаёт уведомление о старте. Все изменения за один цикл объединяются в одно сообщение для каждого чата. Отправка по очереди использует локальные SOCKS5 всех доступных прокси; если все прокси недоступны, используется прямое подключение.

Название подписки показывается в уведомлении только при настройке нескольких источников подписок.
...
Токен бота Telegram, отправляющего уведомления. Требуется указать хотя бы один `TELEGRAM_CHAT_IDS`.
...
Идентификаторы чатов-получателей. Для нескольких чатов укажите `--telegram-chat-id` несколько раз или передайте значения через запятую: `TELEGRAM_CHAT_IDS=123456789,-1001234567890`.

Для отправки в тему forum-группы допишите `message_thread_id` через подчёркивание: `TELEGRAM_CHAT_IDS=-1001234567890_42`. Это идентификатор треда Telegram, который не всегда совпадает с видимым номером темы.
